...
fresh through that provider's own consent screen, with nothing stored.
...
`https://<your-deployment-host>/dns-push/cloudflare/callback`. This needs `CloudflareDns__ClientId`
(the OAuth client's ID) and `CloudflareDns__ClientSecret` (its secret).
...
This needs `AzureDns__TenantId` (your tenant ID), `AzureDns__ClientId` (this app registration's
client ID), and `AzureDns__ClientSecret` (its secret).

Both providers are independently optional. Leave either unset and that provider's push button
simply never appears, with no other effect on the app.

How to actually set these depends on how you're running dotMARC:

- **Docker Compose**: set the matching shell variable before `docker compose up`, the same way as
  the required variables in [Run](#run) above: `CLOUDFLARE_DNS_CLIENT_ID`,
  `CLOUDFLARE_DNS_CLIENT_SECRET`, `AZURE_DNS_TENANT_ID`, `AZURE_DNS_CLIENT_ID`,
  `AZURE_DNS_CLIENT_SECRET` (or the equivalent lines in a `.env` file). `docker-compose.yml` only
  forwards variables it explicitly lists into the container, so setting a `.NET`-style
  `CloudflareDns__ClientId` directly in your shell has no effect, it's the
  `CLOUDFLARE_DNS_CLIENT_ID`-style name compose maps from that matters.
- **Azure**: see [Deploy to Azure](./deploy-to-azure.mdx#optional-dns-provider-push-secrets).
