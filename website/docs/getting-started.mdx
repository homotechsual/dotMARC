...
4. **Running on Azure only:** Container Apps also requires a domain-ownership TXT record before
   ...
   MTA-STS** page, or look it up directly:
...
On the next step, select the **Zone → DNS → Edit** scope and leave it **Required** (not optional).
...
and dotMARC simply reports it can't find a configured push option when you try to use it — the push
