...
`MtaStsManage`), [alerting](./alerts.mdx) (`AlertsView`, `AlertsManage`), DNS provider push
configuration (`DnsPushManage`), and viewing the in-app [server logs](./server-logs.mdx)
(`LogsView`) are each independently grantable.
