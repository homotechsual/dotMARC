...
// The two pills beside each other in the hero share one shape, type and surface so they always match in height.
const heroPillClassName =
  'inline-flex h-9 items-center gap-2 rounded-full border border-[rgba(38,49,65,0.14)] bg-white/80 px-4 text-xs font-extrabold uppercase leading-none tracking-[0.22em] shadow-[0_12px_30px_rgba(38,49,65,0.08)] backdrop-blur dark:border-white/10 dark:bg-[#1e2a3a]/80';
...
function HomepageHeader() {
  ...
  return (
    <header className="relative overflow-hidden pt-6 pb-6 sm:pt-8 lg:pt-10">
      ...
      <div className="container relative mx-auto px-4">
        <div className="max-w-3xl">
          <div className="flex flex-wrap items-center gap-3">
            <div className={`${heroPillClassName} text-[#263141] dark:text-[#fad0cc]`}>
              <span className="h-2 w-2 rounded-full bg-[#e3594f]" />
              {heroCopy.eyebrow}
            </div>
            {latestVersion ? (
              <Link
                className={`${heroPillClassName} text-[#5b6b7d] transition-colors duration-200 hover:border-[#263141]/30 hover:text-[#e3594f] hover:no-underline dark:text-[#9fb0c2] dark:hover:text-[#ef8b86]`}
                to={releaseNotesPath}>
