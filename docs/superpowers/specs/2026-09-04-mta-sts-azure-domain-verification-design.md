...
`AcquireTokenByAuthorizationCode`) happens once per call, before the loop - the authorization code is single-use, so this only works because both
...
`mta-sts` branch always includes the CNAME change as it does today, and - when `MtaStsOptions.Value.Provisioner` is `"Azure"` - also calls the new
