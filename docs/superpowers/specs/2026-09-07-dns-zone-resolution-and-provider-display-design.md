...
**Simplification folded in:** four push-button click handlers across two files (`DomainDetail.razor`'s `PushDmarcRecordAsync`, `PushDmarcAuthorizationRecordAsync`, and `PushTlsrptRecordAsync`; `DomainMtaStsPanel.razor`'s `PushDnsRecordsAsync`) each call `DnsProviderDetector.DetectAsync` live, at click time, purely to pick which OAuth provider (`"cloudflare"`/`"azure-dns"`) to kick off - the push button itself is always shown (gated only on the relevant check's status), never on detection. Three of the four detect against `DomainName`/`Domain.Name`; `PushDmarcAuthorizationRecordAsync` detects against the mailbox's own domain instead, since that record lives in a different zone (see its existing doc comment). All four become reads of the already-loaded `_domain.DnsProvider` (or `Domain.DnsProvider`, passed into `DomainMtaStsPanel` same as today) instead of a fresh live call - safe because the server-side re-detection in "Push-time integration" above is what actually decides the push's `ZoneName`, and already handles a stale client-side value via the `ZoneNotFound` fallback if the two disagree. The DMARC-authorization case still needs its own fresh `DetectAsync(mailboxDomain, ...)` call, unchanged, since that's a different hostname than the one `_domain.DnsProvider` was computed for.
