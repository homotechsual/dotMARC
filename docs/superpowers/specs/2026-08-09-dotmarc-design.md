...
- **`ReportRecord`** - one row per sending source within a report: source IP,
  ...
  Sources tab more readable - e.g. "mail.contoso.io" instead of a bare IP -   but that needs its own decision on how resolution happens and wasn't part
...
exception caught and only logged, invisible in a console-free deployment - twice. Not repeating it here.)
