...
button at top, list below with inline rename and a delete button per row) - duplicated for two sections on one page rather than two separate pages,
...
groups/tags, it reappears whenever any one of its groups/tags is selected - no attempt to dedupe or merge filters; Group and Tag filters are
