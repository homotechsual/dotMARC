...
- **`PollCycle`** - one row per poll cycle that actually ran (a cycle skipped because another
  ...
  were recorded as a `ParseFailure` this cycle), `Succeeded` (bool), `ErrorMessage` (`string?` -   populated only when `Succeeded` is false, i.e. the cycle itself threw outside the existing
...
- `RunPollCycleAsync`, after successfully acquiring the leader lock, calls `PollOnceAsync` and
  ...
  `PollOnceAsync` itself throws (not a per-message failure - those are already caught internally -   but something failing before/outside that loop, e.g. the mailbox fetch), the exception is caught,
